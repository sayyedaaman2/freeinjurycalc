...
function generateSitemap() {
  ...
  // Filter out duplicates and Ontario (which is handled separately)
  const stateSlugs = [...new Set(stateMatches.map(m => m[1]))].filter(slug => slug !== 'canada/ontario');
  ...
  // 2. Static pages
  const staticPages = ['about-us', 'contact-us', 'privacy-policy', 'terms-conditions'];
  ...
  console.log(`Successfully generated sitemap.xml at ${sitemapPath} with ${5 + staticPages.length + stateSlugs.length + specialSlugs.length} URLs.`);
}
